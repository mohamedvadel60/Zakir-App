...
import { X, Download, FileText, RefreshCw, AlertCircle, ExternalLink } from "lucide-react";
...
export const DocumentPreviewModal: React.FC<DocumentPreviewModalProps> = ({
  ...
  useEffect(() => {
    ...
    authenticatedFetch(`/api/auth/verification-document/${documentId}`)
      .then(async (res) => {
        if (!isMounted) return;
        if (!res.ok) {
  ...
  const isImage = mimeType.includes("image") || mimeType.includes("png") || mimeType.includes("jpeg") || mimeType.includes("jpg") || mimeType.includes("webp");
            ...
            <div className="flex items-center gap-2 shrink-0">
              {blobUrl && (
                <a
                  href={blobUrl}
                  download={fileName}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition-colors"
                >
            ...
            {error && !loading && (
              <div className="flex flex-col items-center justify-center gap-3 max-w-md text-center py-10 px-4">
                ...
                <button
                  ...
                  className="mt-2 px-4 py-2 text-xs font-semibold rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 transition-colors"
                >
