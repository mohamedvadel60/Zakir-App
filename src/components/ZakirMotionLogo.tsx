...
/**
 * ZAKIR Official Vector Brand Symbol
 * Exact SVG paths from official brand assets (viewBox: 474.23 0 728.85 810 / 0 0 1435 1199.38)
 */
export const ZakirMotionLogo: React.FC<ZakirMotionLogoProps> = ({
  ...
  if (size === "sm") {
    ...
    symbolPx = size;
    containerPx = Math.round(size * 1.7);
  }

  const isLight = theme === "light";
  const boxBg = isLight ? "#1C2C58" : "#FFFFFF";
  const symbolFill = withBox
    ? isLight
      ? "#FFFFFF"
      : "#1C2C58"
    : isLight
  ...
  if (withBox) {
    return (
      <div
        className={`flex items-center justify-center shrink-0 rounded-2xl select-none transition-colors ${className}`}
        style={{
  ...
  return (
    ...
    >
      ...
      {showText && (
        <span
          className={`font-extrabold tracking-widest uppercase font-display block select-none mt-2 text-lg ${
            isLight ? "text-[#1C2C58]" : "text-white"
          }`}
        >
          ZAKIR
        </span>
