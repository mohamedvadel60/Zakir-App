 ...
 * 
 * Rebuilt strictly against official brand assets:
 * - Light Mode: `Light Mode.txt` (Container #1c2c58, Symbol #fff)
 * - Dark Mode: `Dark Mode.txt` (Container #fff, Symbol #1c2c58)
 * - Exact SVG viewBox: `0 0 1435 1199.38`
 * - Exact 5 Vector Paths from official asset files
 * - Professional SaaS motion easing and seamless 6.0s loop cycle
 */
export const ZakirMotionLogo: React.FC<ZakirMotionLogoProps> = ({
  ...
  // Light Mode: rect fill="#1c2c58", paths fill="#fff"
  // Dark Mode:  rect fill="#fff", paths fill="#1c2c58"
  const containerColor = isLight ? "#1c2c58" : "#ffffff";
  const pathColor = isLight ? "#ffffff" : "#1c2c58";
  ...
  return (
    ...
    >
      {/* Motion Graphic Wrapper */}
      <div
        className="relative flex items-center justify-center"
        style={{ width: `${widthPx}px`, height: `${widthPx}px` }}
      >
        <svg
          id="zakir-motion-svg-root"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 1435 1199.38"
          className="w-full h-full block zakir-motion-root"
          style={{ willChange: "transform, opacity" }}
        >
          <defs>
            <style>{`
              .zakir-bg-rect { fill: ${withBox ? containerColor : "transparent"}; }
              .zakir-path-color { fill: ${pathColor}; }
            `}</style>
          </defs>

          {/* Invisible layout baseline spacer from official asset */}
          <rect y="561.27" width="56" height="56" fill="none" />

          {/* Official Rounded Container Rect (x="235.62", width="1199.38", height="1199.38", rx="262.36", ry="262.36") */}
          {withBox && (
            <rect
              className="zakir-bg-rect zakir-motion-rect"
              x="235.62"
              width="1199.38"
              height="1199.38"
              rx="262.36"
          ...
          {/* Official 5 Vector Paths with Synchronized Motion Choreography */}
          <g className="zakir-motion-paths-group">
            {/* Path 1: Top Apex Node */}
            <path
              className="zakir-path-color zakir-path-1"
              d="M778.63,549.83c-23.73-3.27-49.53-5.85-77.18-7.24-32.15-1.61-62.03-1.36-89.24,0,34.57-18.49,69.14-36.98,103.71-55.47,20.27,4.1,42.87,7.57,67.53,9.65,37.02,3.12,70.37,2.39,98.89,0-34.57,17.69-69.14,35.37-103.71,53.06Z"
            />

            {/* Path 2: Mid-Right Node */}
            <path
              className="zakir-path-color zakir-path-2"
              d="M981.22,706.61c-21.55-3.6-44.07-6.86-67.53-9.65-27.66-3.29-54.23-5.64-79.59-7.24,24.92-14.47,49.85-28.94,74.77-43.41,15.69,5.87,35.17,11.48,57.89,14.47,48.31,6.37,88.45-2.07,113.36-9.65-32.96,18.49-65.92,36.98-98.89,55.47Z"
            />

            {/* Path 3: Upper Sweeping Arc */}
            <path
              className="zakir-path-color zakir-path-3"
              d="M475.93,701.78c-.4-63.54-.79-127.08-1.19-190.62,0-1.55.04-3.1.17-4.65,1.41-16.02,5.73-38.28,19.11-60.39,15.25-25.22,36.03-39.94,52.05-49.5,46.85-27.96,204.08-102.02,420.68-194.1,2.95-1.1,22.35-7.98,38.59,2.41,13.05,8.35,16.88,21.71,19.3,33.77.99,4.94,2.1,13.09,2.12,24.52,0,0-.21,16.41-4.53,33.36-3.15,12.35-9.18,25.15-15.95,36.87-15.62,27.05-38.84,48.89-66.54,63.34-2.2,1.15-4.45,2.31-6.75,3.51-28.28,14.64-50.12,25.26-62.71,31.35-57.8,28-105.19,50.13-105.19,50.14-109.22,51.02-121.47,55.24-155.29,75.28-51.72,30.66-69.24,48.16-82,65.12-22.17,29.46-31.69,59.52-36.18,79.59-.91,5.07-5.6,8.12-9.65,7.24-3.09-.68-5.58-3.59-6.03-7.24Z"
            />

            {/* Path 4: Central Backbone Arc */}
            <path
              className="zakir-path-color zakir-path-4"
      ...
      {showText && (
        <div className="overflow-hidden mt-3">
          <span
            className={`font-extrabold tracking-widest uppercase font-display block select-none ${textSizeClass} zakir-motion-text`}
